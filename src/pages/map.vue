<template>
    ...
    <panContainer class="fill-height flex noSelect" @wheel="wheel" @position="setPosition" @dragend="loadPosition">
      <router-link class="mapRoom noSelect" ondragstart="return false" :to="`/room/${room.name}`" v-for="room in rooms" :key="room.name" :style="{ top: `${room.y}px`, left: `${room.x}px` }" :data-room="room.name">
...
<script>
  ...
  import { roomNameFromXY, roomNameToXY } from '~/assets/utils'
  import Vue from 'vue'
  ...
  function getColor (identifier) {
    ...
    destroyed () {
      ...
      //   }
      // }
      this.$store.dispatch('api/setMapRooms', [])
    ...
    methods: {
      ...
      async loadPosition() {
        ...
        this.roomToView = `/assets/map/${name}.png`
        ...
        console.log('a')
        this.$store.dispatch('api/setMapRooms', rooms)
        console.log('b')
        if (rooms.length) {
